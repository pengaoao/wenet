...
struct PrefixScore {
  ...
  // The start state's id of context graph is 0.
  std::map<int, float> active_states{{0, 0}};  // context search states
  ...
  float context_score() const {
    return std::max(partial_match_context_score, full_match_context_score);
  }
...
class CtcPrefixBeamSearch : public SearchInterface {
 public:
  ...
  explicit CtcPrefixBeamSearch(const std::shared_ptr<ContextGraph> context_graph, const CtcPrefixBeamSearchOptions& opts);
