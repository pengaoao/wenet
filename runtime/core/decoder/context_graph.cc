...
std::pair<float, float> ContextGraph::GetNextContextStates(
    const std::map<int, float>& active_states, const int word_id,
  ...
  for (const auto& state_info : active_states) {
    int state = state_info.first;
    float score = state_info.second;
    // Iterates over arcs of the active state.
    for (fst::ArcIterator<fst::StdFst> aiter(*graph_, state); !aiter.Done();
         aiter.Next()) {
      const fst::StdArc& arc = aiter.Value();
      // The ilabel equals to word id or blank id.
      if (arc.ilabel == word_id || arc.ilabel == 0) {
        ...
        if (graph_->Final(arc.nextstate) == fst::StdArc::Weight::One()) {
          ...
          // Update the the score of the next state in the next active states.
